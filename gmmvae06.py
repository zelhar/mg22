...
class AE_Type600(Generic_Net):
    """
    Autoencoder with cluster center parameter.
    """
    def __init__(
        self,
        nx: int = 28 ** 2,
        ...
        self.mz = nn.Parameter(
                torch.randn(nclasses, nz), requires_grad=True,)
        return
    ...
    def forward(self, input, y=None):
        ...
        total_loss = (
                ...
                + loss_l * 1e0
                + loss_cluster * 1e0
                #- (q * q.log()).sum(-1).mean() * 1e2
                )
        losses["total_loss"] = total_loss
        output["losses"] = losses
        return output

class AE_Type603(nn.Module):
    """
...
class VAE_Dilo_Type601(nn.Module):
    """
