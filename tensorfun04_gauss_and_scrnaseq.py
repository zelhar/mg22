...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        if self.is_Bernoulli:
            ...
            mu = self.xmu(h)
            logvar = torch.exp(self.xlogvar(h))
            return mu, logvar
...
class VAE(nn.Module):
    ...
    def model(self, x):
        ...
        with pyro.plate("data", x.shape[0]):
            ...
            #scale_img = logvar_img.exp()
            scale_img = logvar_img.clip(0,4) # use it as scale b/c of softmax activation
...
x = imgs.flatten(1).cuda()

mu, logvar = model.encoder(imgs.cuda())
z = distributions.Normal(mu, logvar.exp()).sample()
loc_im, loc_lv = model.decoder(z.cuda())
...
plot_images(denorm(imgs.view(-1,1,28,28)))

plot_images(loc_im.view(-1,1,28,28))

plot_images(denorm(loc_im.view(-1,1,28,28)))
