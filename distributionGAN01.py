...
netG = LinGenerator(nin=nz, nh=3000, nout=2).to(device)

netD = LinDiscriminator(nin=2, nh=3000).to(device)
...
noise = sampler.sample((50000,nz)).to(device)
...
z = gmm.sample((100*batch_size,)).clip(-1,1).numpy()
z.shape
x = z[:,0]
y = z[:,1]
plt.scatter(x,y)
...
class SimpleDecoder(nn.Module):
    def __init__(self, nin=nz, nh=512, nout=10):
...
encoder = SimpleEncoder(nin=2, nout=nz).to(device)
decoder = SimpleDecoder(nin=nz, nout=2).to(device)
...
bar = sampler.sample((128,2)).to(device)
foo = gmm.sample((128,)).to(device)
    ...
    #for i, data in enumerate(train_loader, 0):
        ...
        if i % 50 == 0:
            print('[%d/%d][%d/%d]\tLoss_D: %.4f'
                  % (epoch, num_epochs, i, rounds , loss.item()))


z = gmm.sample((100*batch_size,)).clip(-1,1)
x = z.numpy()[:,0]
y = z.numpy()[:,1]
plt.scatter(x,y)

zz = decoder(encoder(z.cuda())).detach().cpu()
