...
Instructor / 1st Supervisor: professor Martin Vingron\\
...
linktodoc=true,
]{./Selbstaendigkeitserklaerung_EN.pdf}
...
number of axes but it is inconsistent with the way dimension is used in
...
independent replications of a random vector $\x$. This is another motivation why
...
everything factors nicely, e.g. $p(\X | \Z) = \prod_i p(\x_i | \z_i)$.
...
mode and achieved $0.9$ accuracy. The best classifier listed on the
...
We also want to see whats going on in the $w$ space. There we ecpect everything
