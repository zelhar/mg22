...
Then a little bit more research got me into the Gaussian mixture model VAE of
...
network to define the classifier $q(\y|\z)$. Actually their factorization is
given as $q(\y|\z,\w)$ but more importantly they rely they define
$q(\y|\z, \w) \triangleq p(\y | \z, \w)$ (they don't have Dirichlet prior $d$)
...
doesn't use this tractability in $q$.

In \gmvae with the addition of a Dirichlet prior, we have $q(\y | \d)$ and 
it isn't possible, or at least not immediately obvious, how to use $p(\y | \w,
\z)$ in the $q$ factorization.
Moreover $p$ and $q$ are two separate distributions, and even though $p(\y | \w,
\z)$ is tractable when $\y$ happens to have discrete distribution, it doesn't
mean we must impose it on $q$.
\end{remark}
%\end{framed}
