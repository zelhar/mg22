...
%\chapter{Neural networks, autoencoders,  variance inference and variational autoencoders}
...
\begin{aligned}
\mathcal{L}(q) &\triangleq \int \log \frac{p(\bv{X},\bv{Z})}{q(\bv{Z})} d
q(\bv{Z})
...
\log p(\bv{X}) = \mathcal{L}(q) - KL(q(\bv{Z}) \| p(\bv{Z}|\bv{X}))
\end{equation}
...
\begin{equation}
\label{eq:alphahat}
\hat\alpha \triangleq \text{arg max}_{\alpha}(\mathcal{L}(q_{\alpha}) 
\end{equation}
