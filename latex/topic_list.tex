...
We need to choose points $\z \R^m$ from the latent space and project back to
observed space $\psi(\z)$. The question is then how to sample these $\z$?
It's not immediately clear how to do this. Perhaps $\phi(X)$ can be be
approximated by some distribution in $\z$ which we can sample from.

If we use a VAE instead, we know that $\z$ should have a distribution pretty
...
However $q(\x | \z)$ is some kind of simple distribution for example 
$\NN(\z: \mu(\x), \sigma(\x))$ so we can use the known mean $\mu(\x)$ as the
encoding. Or we can deliberately add stockanticity and 
