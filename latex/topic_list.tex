...
We want to create some mixture distribution, where the inferred category random
...
There are several advantages to doing this. For one, we are interested in
dimensionality reduction and not just unsupervised classification, in that
case it makes sense to create a mixture distribution in the latent space which
we can than plot for example with UMAP.
A second reason is that perhaps it is easier to create a mixture Gaussian on
the simpler, lower dimensional space $\z$ than on the $\x$ space where the data
probably occupies a complex hyper-surface.
...
Figure~\ref{fig:vanillamix} describes the usual mixture model. The components'
...
GMMVAE~\cite{dilokthanakul2016deep}, where $\pi$ is a fixed parameter and resides
...
it, is that it imposes a "bad categorical prior" on $\y$, because $p(\y) \sim
...
good predictor so we don't want to impose it to be close to uniform. If we move
$\phi$ inside and give it a symmetric Dirichlet prior, we allow it more
flexibility to be heavily biased towards one category, making it a good
predictor. $\alpha$ is a symmetric Dirichlet hyper-parameter.
...
Our DGMMMVAE model is based on figure~\ref{fig:mymix}.
