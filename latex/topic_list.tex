...
:\R^n \to \R^m$ is parameterized, it means that implicitly has additional 
...
there are plenty of other activation functions.
...
and $\rho: \R^m \to \R^l$ are NNs and their parameter sets are discjoint then $\phi = \rho
\circ \psi$ is a NN.
...
the $\psi_i$'s then
$\phi = \nu(\psi_1, \dots, \psi_n)$ is a NN if the diemnsions "make sense".
\end{enumerate}
...
resulting network is hirarchical. Its graph can partitioned into \emph{levels}
$l_0, l_1\dots$ and there are only directed edges between two consequtive
levels.
...
The incoming edges into a node represent linear function, but we will in general let
it represent an affine function without bothering with adding a fixed $1$ input
node. A Node's output is therefore only dependent on those nodes which have
directed edges connecting to it.
