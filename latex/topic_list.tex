...
%Perhaps $\phi(X)$ can be be
%approximated by some distribution in $\z$ which we can sample from.
...
that $\x$ has mixture distribution.
We would like to use a VAE with a latent space $\z$ and it too should have 
mixture distribution. One challenge that needs to be sorted is how to define
a prior distribution that is a mixture type without having access to the
categorical information.

One model which is called the Gaussian mixture model was proposed by 
Dilokthanakul et.al~\cite{dilokthanakul2016deep} and we base our proposed model
on their model with some adaptations and present it in the next section.

\section{The GMMVAE model}
...
reconstruct $\x$ from.
One way we could try is to make $p(\x | \z, \y)$ a mixture distribution, as the
M2 model in~\cite{kingma2014semi}.

We assume that $\x$ has some sort of Gaussian mixture distribution $p(\x|\y)$,
but of course $\y$ is unknown. We will have a latent $\z$ and so $p(\x | \z)$
will need to be 
some sort of Gaussian mixture distribution. 
Another random variable, $\w$ is used assist in creating the
mixture in $\z$, 
with $\z | \y$ having diagonal Gaussian distribution.
Finally we use another random variable $\dd$ to
create a "good" prior" which is a symmetric Dirichlet type, 
for the categorical distribution $\y$.
$\dd$ and $\w$ require some fixed prior distributions, whereas the priors for 
$\y$ and $\z$ will be learned as we'll see.
We have a hyper-parameter $\alpha$ which we use for the prior of $\dd$.
$\w$ will have standard normal diagonal distribution as its prior.
...
\item{} draw a categorical distribution from the prior $\dd \sim p(\dd) = \text{Dir}(\alpha)$
\item{} draw a category $\y \sim p(\y | \d) = \text{Cat}(\dd)$
\item draw a latent $\z|\y \sim p(\z | \w,\y) = \NN(\z;\bv{\mu}(\w,\y), \bv{\sigma}(\w,\y)$
