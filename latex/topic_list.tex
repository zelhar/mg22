...
When our data comes in several matrices, for example input data $\X$ and target
matrix $\Y$, it means they have the same number of rows. And when we speak about 
...
We assume that
the $\bv{x}_i$'s are i.i.d with some distribution function $p(\x)$ and therefore for
...
the the VAR maximizes equation~\ref{eq:elbo_conditioned} and therefore also
the log evidence.

\subsection{Computing the ELBO}
The quesion is how to compute the integral in the ELBO function. 
Recall that given an input $\x \R^n$,
...
$\z \in \R^m$ rather then a concrete denterministic point:
$\z \sim q(\z | \x)$. Suppose that we draw one concrete sample $\z \in \R^m$
taken from that distribution. Now we have the concrete input $\x$ and the
...
Now that we have a concrete $\z$, and $\x$ we can also compute $\log p(\x | \z)$
...
More generally, instead of drawing just one concrete $\z$, we draw $k$ samples
